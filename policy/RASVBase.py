...
class rasActorPolicy(ABC, nn.Module):
    ...
    * :meth:`~tianshou.policy.BasePolicy.update`: the main interface for training, \
    ...
    def compute_nstep_return(
        ...
        # Collect rewards, reach, avoid from environment
        rew = buffer.rew
        reach = buffer.info.reach
        avoid = buffer.info.avoid
        ...
        target_q = target_q * rasActorPolicy.value_mask(buffer, terminal).reshape(-1, 1)
        end_flag = buffer.done.copy()
        end_flag[buffer.unfinished_index()] = True
        target_q = _nstep_return(reach, avoid, rew, end_flag, target_q, indices, gamma, n_step)
    ...
    def _compile(self) -> None:
        ...
        _nstep_return(f64,f64,f64, b, f32.reshape(-1, 1), i64, 0.1, 1)
...
def _nstep_return(
    ...
    # Bellman Equation
        ...
        # The functioin is negated to represent a reward instead of a cost
        # target_q: accumulated future reward from action, H(f(x,u))
