# Gabriel Chenevert
# 2/18/2024
# This is an example of a DDPG Reach Avoid Stay implementation using Tianshou
# It will use the pendulum gymnasium environment
...
def get_args():
    ...
    parser.add_argument('--task', type=str, default='DroneChase-v1')   # Set the task to be the Fly environment
...
def V_ddpg(args=get_args(), task = None, epoch = None, do_term = None, term_reward = None, logdir = None, batch_size = None, hidden_sizes = None, **kwargs):
    ...
    if args.reward_threshold is None:
        default_reward_threshold = {"RASFly-v1": -250}
        args.reward_threshold = default_reward_threshold.get(
            task, env.spec.reward_threshold
        )

    # train_envs = gym.make(task)torch.load(criticPath)
    train_envs = SubprocVectorEnv(
        [lambda: gym.make(task,hPolicy = hPolicy, do_term = do_term, term_reward = term_reward) for _ in range(args.training_num)],
        context = 'spawn',share_memory=True
    )
    # test_envs = gym.make(task)
    test_envs = SubprocVectorEnv(
        [lambda: gym.make(task,hPolicy = hPolicy, do_term = do_term, term_reward = term_reward) for _ in range(args.test_num)],
        context = 'spawn',share_memory=True
    )
    # seed
    ...
    def save_best_fn(policy):
        ...
        torch.save(policy, os.path.join(log_path, 'vPolicy.pth'))
